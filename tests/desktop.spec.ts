...
test.describe('Desktop - Full Editor Flow', () => {
  test('should complete all 4 steps from home to download', async ({ page }) => {
    ...
    // Wait for AI background removal to complete (first run downloads ~25MB WASM model)
    // Timeout set to 3 minutes for model download + processing
    ...
    // Choose "템플릿 배경" (template background) for faster processing
    ...
    // ============ STEP 3: Canvas Composition (Konva Editor) ============
    // Canvas appears immediately after Step2 background selection
    const canvas = page.locator('canvas').first();
    await expect(canvas).toBeVisible({ timeout: 10000 });

    // Click "다음 단계로" to proceed to Step4
